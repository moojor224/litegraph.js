...
ContextMenu
*/
type Class<T = any> = T & { new (): T };
class LGraphNode {
    static type = "";
    constructor() {}
}
const node = new LGraphNode();
interface LGraphNodeConstructor<T extends LGraphNode> {
    new (...args: any[]): T;
}
export const LiteGraph = {
    ...
    NODE: {
        ...
        DEFAULT_BGCOLOR: "#353535",
        DEFAULT_BOXCOLOR: "#666666",
        DEFAULT_SHAPE: "box",
    ...
    LINK_COLOR: "#99AA99",
    EVENT_LINK_COLOR: "#A/86a86",
    CONNECTING_LINK_COLOR: "#AAFFAA",

    /** maximum number of consecutive node executions allowed to avoid infinite loops */
    MAX_NUMBER_OF_NODES: 1000,
    /** default node position in LGraphCanvas */
    ...
    NODE_MODES: {
        NAMES: ["Always", "On Event", "Never", "On Trigger"], // helper, will add "On Request" and more in the future
        COLORS: ["#666666", "#442222", "#333333", "#222244", "#662266"], // use with node_box_coloured_by_mode
        ALWAYS: 0,
        ON_EVENT: 1,
        NEVER: 2,
        ON_TRIGGER: 3
    ...
    registerNodeType: function (type: string, base_class: typeof LGraphNode) {
        if (!base_class.prototype) {
            throw "Cannot register a simple object, it must be a class with a prototype";
        }
        if (!(base_class.prototype instanceof LGraphNode)) {
            console.error(`Class '${base_class.name}' must extend LGraphNode`);
            return;
        }
                ...
                set: function (v) {
                    switch (v) {
                        case "default":
                            ...
                            break;
                        case "box":
                            this._shape = LiteGraph.NODE.SHAPES.BOX_SHAPE;
                            break;
                        case "round":
                            this._shape = LiteGraph.NODE.SHAPES.ROUND_SHAPE;
                            break;
                        case "circle":
                            this._shape = LiteGraph.NODE.SHAPES.CIRCLE_SHAPE;
                            break;
                        case "card":
                            this._shape = LiteGraph.NODE.SHAPES.CARD_SHAPE;
                            break;
                        default:
                            this._shape = v;
                    }
     ...
     */
    unregisterNodeType: function (type: string | object) {
     ...
     */
    buildNodeClassFromObject: function (name: string, object: object) {
        var ctor_code = "";
        if (object.inputs)
     ...
     * @param {Function} func
     */
    addNodeMethod: function (name, func: Function) {
     ...
     */
    getNodeType: function (type: string): Class {
    ...
    getNodeTypesInCategory: function (category: string, filter): Array<any> {
        var r = [];
        for (var i in this.registered_node_types) {
            var type = this.registered_node_types[i];
     ...
     */
    getNodeTypesCategories: function (filter: string): Array<any> {
        var categories = { "": 1 };
        for (var i in this.registered_node_types) {
            var type = this.registered_node_types[i];
     ...
     */
    isValidConnection: function (type_a: string, type_b: string): boolean {
     ...
     * @return {Boolean} true if they can be connected
     */
    registerSearchboxExtra: function (
        node_type: string,
        description: string,
        data: object
    ): boolean {
